...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const bytes = await downloadImage(data.imageUrl, join(outDir, 'assets', 'product.png'));
  data.imageFile = 'assets/product.png';
  ...
  console.log(`    ảnh ${(bytes / 1024).toFixed(0)} KB -> ${outDir}/assets/product.png`);
}
