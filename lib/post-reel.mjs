...
// Comment link mua. Reel là một Video node; bài đăng tương ứng nằm ở field post_id.
// Nếu bước này hỏng, reel vẫn đã lên — chỉ cảnh báo, không đánh sập workflow.
try {
  console.log('comment  lấy post_id');
  const meta = await jsonOrThrow(
    await fetch(`${GRAPH}/${video_id}?fields=post_id&access_token=${TOKEN}`),
    'post_id'
  );
  const target = meta.post_id ?? video_id;
  console.log(`         target=${target}`);

  const body = new URLSearchParams({ message: buildComment(data) });
  await jsonOrThrow(
    await fetch(`${GRAPH}/${target}/comments`, {
      method: 'POST',
