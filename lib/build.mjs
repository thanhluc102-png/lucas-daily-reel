...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (voiceFile) {
    // Căn để lời đọc KẾT THÚC gần cuối video -> câu CTA ("Mua ngay...") khớp cảnh CTA,
    // không bị hụt đuôi (giọng đọc xong sớm rồi im). Đo độ dài voice bằng ffprobe.
    let vDur = total, vStart = 0.15;
    try {
      const d = parseFloat(
        execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${join(outDir, voiceFile)}"`).toString().trim()
      );
      if (d > 0.5 && d <= total) { vDur = d; vStart = Math.max(0, total - d - 0.25); }
    } catch (err) {
      console.warn(`[voice] không đo được độ dài, giữ mặc định: ${err.message}`);
    }
    console.log(`voice sync: start=${vStart.toFixed(2)}s  dur=${vDur.toFixed(2)}s  (video ${total}s)`);
    tracks.push(`<audio id="voice" data-timeline-role="voice" data-start="${vStart.toFixed(2)}" data-duration="${vDur.toFixed(2)}" data-track-index="2" data-volume="1" src="${voiceFile}"></audio>`);
  }
