...
// Xen kẽ: mỗi nhịp luân phiên 1 "hàng mới về" (theo ngày), 1 "đang giảm giá".
// Luân phiên dựa vào nguồn (src) của bài đăng gần nhất trong history.
async function pickMix(history, seen, excluded) {
  const clean = (list) =>
    list.filter((p) => p.is_in_stock && !excluded.has(p.id) && p.images?.[0]?.src);
  const lists = {
    new: clean(await listProducts(STRATEGIES.newest)), // hàng mới về (mới nhất theo ngày)
    onsale: clean(await listProducts(STRATEGIES.onsale)), // đang giảm giá
  };

  // Lần trước là 'new' -> lần này ưu tiên 'onsale', và ngược lại. Bắt đầu bằng 'new'.
  const wantNew = history.used?.[0]?.src !== 'new';
  const order = wantNew ? ['new', 'onsale'] : ['onsale', 'new'];

  for (const src of order) {
    const list = lists[src];
    const fresh = list.filter((p) => !seen.has(p.id));
    // Còn hàng chưa quay -> mới nhất; hết -> bốc ngẫu nhiên (tránh lặp cùng 1 sp).
    ...
    if (chosen) {
      return {
        ...
        name: chosen.name,
        src,
        reason: `mix -> ${src === 'new' ? 'hàng mới về' : 'đang giảm giá'}` +
          ` (${list.length} sp hợp lệ, ${fresh.length} chưa quay ${CONFIG.historyDays ?? 21} ngày)`,
      };
    }
  }
