...
const DEFAULT_VOICE = 'ZsjEJaLQy3sgvwxicmDx';

// voiceId truyền vào (build.mjs bốc từ pool) > env > mặc định.
export async function generateVoice(text, outDir, voiceId) {
  const key = process.env.ELEVENLABS_API_KEY;
  if (!key) throw new Error('thiếu ELEVENLABS_API_KEY');
  const line = (text ?? '').trim();
  if (!line) throw new Error('không có lời đọc');
  const voice = voiceId ?? process.env.ELEVEN_VOICE_ID ?? DEFAULT_VOICE;
